...
export default function MobileMemoPage() {
  ...
  const canRequest = !!token && !!ownerType && Number.isFinite(ownerId) && ownerId > 0;
  ...
  const loadMemoDetail = useCallback(
    async (memoId: number) => {
      ...
      const res = await fetch(`/api/memos/${memoId}?owner_type=${ownerType}&owner_id=${ownerId}`, { headers });
      if (!res.ok) return;
  ...
  const loadMemos = useCallback(async () => {
    ...
    try {
      ...
      const activeId = selectedMemoId && next.some((memo) => memo.memo_id === selectedMemoId) ? selectedMemoId : next[0].memo_id;
      setSelectedMemoId(activeId);
      await loadMemoDetail(activeId);
    } finally {
      setLoading(false);
    }
  }, [canRequest, headers, loadMemoDetail, ownerId, ownerType, selectedMemoId]);
  ...
  const createMemo = useCallback(async () => {
    ...
    await loadMemos();
  }, [canRequest, headers, loadMemos, ownerId, ownerType]);
  ...
  if (!canRequest) {
    return <main className="min-h-screen p-4 text-sm text-red-500">인증 또는 워크스페이스 정보가 없습니다.</main>;
  }
    ...
    <main className="min-h-screen bg-background p-3 text-foreground">
      <section className="mx-auto flex max-w-5xl flex-col gap-3">
        <div className="rounded-2xl border border-border bg-card p-3">
          ...
          <p className="mt-1 text-xs text-muted-foreground">{saving ? "저장 중..." : message || "자동 저장"}</p>
        </div>

        <div className="grid gap-3 md:grid-cols-[220px_1fr]">
          <div className="rounded-2xl border border-border bg-card p-2">
            <div className="max-h-[44vh] space-y-1 overflow-auto">
              {loading ? <p className="px-2 py-3 text-sm text-muted-foreground">불러오는 중...</p> : null}
              {!loading && !memos.length ? <p className="px-2 py-3 text-sm text-muted-foreground">메모가 없습니다.</p> : null}
              {memos.map((memo) => {
                const active = memo.memo_id === selectedMemoId;
                return (
                  <button
                    key={memo.memo_id}
                    type="button"
                    ...
                    className={`w-full rounded-xl border px-3 py-2 text-left text-sm ${active ? "border-primary bg-primary/10" : "border-border bg-card"}`}
                  >
                    <p className="truncate font-semibold text-foreground">{memo.title || "제목 없음"}</p>
                    <p className="mt-1 truncate text-xs text-muted-foreground">{new Date(memo.updated_at).toLocaleString()}</p>
                  </button>
                );
              })}
            </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-3">
            ...
            <RichTextEditor
              ...
              contentKey={selectedMemoId ?? "none"}
