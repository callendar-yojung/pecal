...
export default function TaskFormPanel({
    ...
    try {
      ...
      if (!response.ok) {
        if (data?.code === "LIMIT_EXCEEDED") {
          const used =
            typeof data.used_bytes === "number"
              ? formatBytes(data.used_bytes)
              : null;
          const limit =
            typeof data.limit_bytes === "number"
  ...
  }, [isOpen, workspaceType, ownerId, fetchTags]);

  useEffect(() => {
    if (isOpen && initialData?.id) fetchAttachments(initialData.id);
    else setAttachments([]);
  }, [isOpen, initialData?.id, fetchAttachments]);
  ...
  return (
    <div
      className={
        isPage
          ? "w-full"
          : "fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 overflow-y-auto"
      }
    >
      <div
        className={`w-full bg-card border border-border shadow-2xl transition-all ${isPage ? "rounded-xl p-8" : "max-w-3xl rounded-2xl p-6 my-auto"}`}
      >
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div key={field.id} className="space-y-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    value={
                      splitDateTime(
                        formData[field.id as keyof TaskFormData] as string,
                      ).date
                    }
                    onChange={(e) =>
                      setDateTime(field.id as any, e.target.value, "")
                    }
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <button
                        type="button"
                        className="inline-flex h-10 w-28 items-center justify-between rounded-xl border border-input bg-background px-3 text-sm hover:bg-muted transition-colors"
                      >
                      ...
                      <div className="max-h-60 overflow-y-auto no-scrollbar">
                        {timeOptions.map((time) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setDateTime(field.id as any, "", time)
                            }
                ...
                {errors[field.id as keyof TaskFormData] && (
                  <p className="text-xs font-medium text-destructive">
                    {errors[field.id as keyof TaskFormData]}
                  </p>
                )}
          ...
          <div className="space-y-2">
            ...
            <div className="rounded-xl border border-input bg-background overflow-hidden focus-within:ring-2 focus-within:ring-primary/20 transition-all">
              <RichTextEditor
                ...
                onChange={(json) =>
                  handleChange("content", JSON.stringify(json))
                }
              />
            </div>
          </div>
                ...
                {colorOptions.map((color) => (
                  ...
                  >
                    {formData.color === color.value && (
                      <Check
                        size={14}
                        className="mx-auto text-white drop-shadow-md"
                      />
                    )}
                  </button>
                ))}
          ...
          <div className="space-y-3 rounded-xl bg-muted/30 p-4 border border-border/50">
            ...
            <div className="flex flex-col gap-2">
              ...
              <label
                htmlFor="file-upload"
                className="flex cursor-pointer items-center justify-center rounded-xl border border-dashed border-input bg-background p-3 text-sm text-muted-foreground hover:bg-muted transition-colors"
              >
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {pendingFiles.map((file) => (
                  <div
                    ...
                    className="group flex items-center justify-between rounded-lg border bg-background px-3 py-2 text-xs"
                  >
